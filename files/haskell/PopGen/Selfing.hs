...
afs2 thetas ps = Distribution "afs2" (make_densities $ ewens_diploid_probability thetas ps) (error "afs2 has no quantile") () ()

robust_diploid_afs n_individuals n_loci s f theta_effective = lazy $ do
    t <- iid n_individuals (rgeometric s)

    i <- (independent [ iid n_loci $ rbernoulli $ 0.5 ** t !! k * (1.0 - f) | k <- [0 .. n_individuals - 1] ])
