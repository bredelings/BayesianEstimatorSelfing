...
andro_mating_system' s p_m = 1 / ((1 + s) ^ 2 / (4 * p_h) + (1 - s) ^ 2 / (4 * p_m)) where p_h = 1 - p_m
...
andro_mating_system2' s p_m sigma = r  where
    p_h = 1 - p_m                   -- fraction of hermaphrodites
    mp  = p_m * sigma               -- male pollen
    hp  = p_h                       -- herm pollen
    g_m = 0.5 * (1 - s) * mp / (mp + hp)   -- probability a parent is male
    g_h = 1 - g_m                   -- probability a parent is herm
